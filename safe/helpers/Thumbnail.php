...
class Thumbnail {
        ...
        if($this->error == false) {
            switch($this->format) {
                case 'GIF':
                    ...
                    break;
                case 'JPG':
					ini_set('gd.jpeg_ignore_warning', 1);//Esta liena es para evitar problemas con imagenes jpg cargadas desde mac o algunos programas
    ...
    /**
     * Toma la mayor proporción de una imagen para luego sacar el resize
     *
     * @param int $maxWidth
     * @param int $maxHeight
     */
    function cropResize($maxWidth = 0, $maxHeight = 0) {
    	$width = $this->currentDimensions['width'];
    	$height = $this->currentDimensions['height'];
		...
		if($ratioHeight < $ratioWidth)
			$this->crop(0, 0, floor($maxWidth*$ratioHeight), $height);
		else
			$this->crop(0, 0, $width, floor($maxHeight*$ratioWidth));
